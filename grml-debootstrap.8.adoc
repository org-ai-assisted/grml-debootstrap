...
range (0, INT64_MAX):
  ...
  SOURCE_DATE_EPOCH=1767225600 grml-debootstrap --vm --vmfile \
...
*e2fsprogs* >= 1.44.0 (mke2fs SOURCE_DATE_EPOCH support).
...
with a fixed salt derived from *ROOTPASSWORD_SEED*; when ROOTPASSWORD_SEED is unset
the salt is derived from SOURCE_DATE_EPOCH instead. That means anyone, anywhere,
who builds an image with the same SOURCE_DATE_EPOCH uses the same salt to hash
their root password as every other such user, and building several images that
share a SOURCE_DATE_EPOCH reuses the salt across them. This is not a problem in
most scenarios but may be in some. Users who consider it a problem must not use
reproducible builds, or must set *ROOTPASSWORD_SEED* to a fixed but initially
randomly generated value. grml-debootstrap logs a warning at build time when
SOURCE_DATE_EPOCH is set but ROOTPASSWORD_SEED is not.
