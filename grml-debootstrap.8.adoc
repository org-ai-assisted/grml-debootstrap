...
*--imagesize* _size_::
   ...
   Use specified size for size of Virtual Machine or Raspberry Pi disk file.
   If not specified it defaults to 2G (being 2GiB) for virtual machines, and 3G
   (being 3GiB) for Raspberry Pi images. Syntax as supported by qemu-img (see
   manpage qemu-img(1) for details.
   Usage example: --imagesize 3G
...
*--rpi*::
    ...
    Set up a Raspberry Pi installation on an existing block device, which will
    be partitioned. This allows deployment of a physical Raspberry Pi system.
    The option needs to be combined with the --target option.
    This option automatically enables the --defaultinterfaces option.
    Usage example: --rpi --target /dev/mmcblk0

*--rpifile*::

    Set up a Raspberry Pi installation using a regular file instead of
    installing to a partition/block device. This allows creation of a
    Raspberry Pi image. The option needs to be combined with the --target
    option ('qemu-img create -f raw ...' is executed on the specified target).
    This option automatically enables the --defaultinterfaces option.
    Usage example: --rpifile --target /mnt/sda1/rpi.img

*--rpifirm* _device_::

    Partition where Raspberry Pi firmware should be installed to. If the
    specified device doesn't use a FAT filesystem yet grml-debootstrap
    automatically creates the filesystem. If a FAT filesystem is already
    present the filesystem creation is skipped. Please ensure that it's a
    valid FAT32 filesystem. This option is NOT supported for Raspberry Pi
    image file deployments.
...
*--vmsize* _size_::
   ...
   Deprecated alias for --imagesize.
...
grml-debootstrap can build bit-for-bit reproducible Virtual Machine images
(*--vm* / *--vmfile*). Reproducible builds are enabled solely by exporting
*SOURCE_DATE_EPOCH*, the tool-agnostic switch used across the reproducible-builds
ecosystem; there is no separate on/off flag. Set it to a decimal integer in the
range (0, INT64_MAX):

  SOURCE_DATE_EPOCH=1767225600 grml-debootstrap --vmfile \
    --target /tmp/disk.img --release trixie

Disk and filesystem identifiers, the filesystem block layout and all embedded
timestamps are then derived deterministically from it, so two builds with the same
SOURCE_DATE_EPOCH and inputs produce byte-identical images. Leave SOURCE_DATE_EPOCH
unset for a normal, non-reproducible build. A set but malformed value is a hard
error rather than a silently non-reproducible build. Reproducible EFI images
additionally require the *mtools* package, and reproducible builds require
*e2fsprogs* >= 1.47.1 (mke2fs SOURCE_DATE_EPOCH support).

The package set is pinned as well. A reproducible build ignores both the default
mirror and *--mirror* and installs from the snapshot.debian.org archive as it stood
at SOURCE_DATE_EPOCH, so the same SOURCE_DATE_EPOCH keeps selecting the same package
versions as the Debian archive moves on. The configured mirror is written into the
installed system at the end of the build, so the resulting image tracks a real
mirror and not the snapshot. Set *SNAPSHOT_ARCHIVE* to use a local snapshot mirror
instead of snapshot.debian.org.

WARNING: enabling reproducible builds introduces a security risk when a root
password is specified and relied upon for security. The root password is hashed
with the fixed salt *ROOTPASSWORD_SALT*; when ROOTPASSWORD_SALT is unset the salt is
derived from SOURCE_DATE_EPOCH instead. That means anyone, anywhere, who builds an
image with the same SOURCE_DATE_EPOCH uses the same salt to hash their root password
as every other such user, and building several images that share a SOURCE_DATE_EPOCH
reuses the salt across them. This is not a problem in most scenarios but may be in
some. Users who consider it a problem must not use reproducible builds, or must set
*ROOTPASSWORD_SALT* to a fixed but initially randomly generated value.
grml-debootstrap logs a warning at build time when SOURCE_DATE_EPOCH is set but
ROOTPASSWORD_SALT is not.
...
Use specified mirror instead of the default (http://deb.debian.org/debian) one.
  ...
  grml-debootstrap --vmfile --imagesize 3G --target /mnt/sda1/qemu.img
